...
XPerl_SetModuleRevision("$Revision: 1229 $")
...
function XPerl_MinimapMenu(self)
	if (not XPerl_Minimap) then
		CreateFrame("Frame", "XPerl_Minimap", nil, BackdropTemplateMixin and "BackdropTemplate")
...
local function GetFreeFader(parent)
	...
	if (bar) then
		...
		bar = CreateFrame("StatusBar", nil, parent, BackdropTemplateMixin and "BackdropTemplate")
	end
...
function XPerl_GetBuffButton(self, buffnum, debuff, createIfAbsent, newID)
	...
	if (not button and createIfAbsent) then
		...
		button = CreateFrame("Button", "XPerlBuff"..buffIconCount, parent, format("BackdropTemplate,XPerl_Cooldown_%sTemplate", buffType))
		button:Hide()
...
local function AuraButtonOnShow(self)
	...
	if (not cd) then
		cd = CreateFrame("Cooldown", nil, self, "BackdropTemplate,CooldownFrameTemplate")
		self.cooldown = cd
		cd:SetAllPoints()
	end
	cd:SetReverse(true)
...
function XPerl_Unit_ThreatStatus(self, relative, immediate)
	...
	if (not t) then
		...
		if (self[mode]) then -- If desired parent frame exists
			t = CreateFrame("Frame", self:GetName().."Threat"..mode, self[mode], "BackdropTemplate,XPerl_ThreatTemplate"..mode)
			t:SetAllPoints()
